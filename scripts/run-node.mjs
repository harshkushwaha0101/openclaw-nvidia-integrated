...
const DEFAULT_BUILD_LOCK_STALE_MS = 2 * 60 * 1000; // 2 minutes: on Windows, stale locks accumulate; be aggressive
...
const removeStaleBuildLock = (deps, lockDir, staleMs) => {
  try {
    ...
    // On Windows, add aggressive retry for lock removal
    if (process.platform === "win32") {
      let lastError;
      for (let attempt = 0; attempt < 5; attempt++) {
        try {
          deps.fs.rmSync(lockDir, { recursive: true, force: true, maxRetries: 3, retryDelay: 50 });
          return true;
        } catch (err) {
          lastError = err;
          if (attempt < 4) {
    ...
    // Log stale lock cleanup failures for debugging
    if (error?.message) {
      const logMsg = `[openclaw] Warning: failed to remove stale build lock: ${error.message}\n`;
      if (deps.stderr && typeof deps.stderr.write === "function") {
        deps.stderr.write(logMsg);
      }
    }
    return false;
  }
...
const acquireRunNodeBuildLock = async (deps) => {
  ...
  // On Windows, try to clean up any stale locks before attempting acquisition
  if (process.platform === "win32" && deps.fs.existsSync(lockDir)) {
    try {
      removeStaleBuildLock(deps, lockDir, staleMs);
    } catch {
