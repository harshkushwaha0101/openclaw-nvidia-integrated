...
function copyDirRecursively(sourcePath, targetPath, attempts = 3) {
  // Fallback for Windows: use cp -r or node API with retries
...
function renameWithRetry(sourcePath, targetPath, maxRetries = 25, delayMs = 400) {
  let lastError;
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      // On Windows, ensure target doesn't exist or is removable before rename
      ...
      lastError = error;
      // Retry on Windows permission/lock errors
      if (process.platform === "win32" && (error.code === "EPERM" || error.code === "EACCES" || error.code === "ENOTEMPTY") && attempt < maxRetries - 1) {
        const backoffMs = delayMs * Math.pow(2, Math.min(attempt, 5));
        // Wait with exponential backoff, up to ~12.8 seconds total
        const endTime = Date.now() + backoffMs;
        while (Date.now() < endTime) {
          // Minimal busy-wait to yield CPU
        }
        continue;
      }
      throw error;
  ...
  try {
    if (fs.existsSync(targetPath)) {
      try {
        ...
        // If rename fails on Windows, use copy as fallback
        if (process.platform === "win32") {
          if (copyDirRecursively(targetPath, backupPath)) {
    ...
    // Try rename first, fallback to copy on Windows
    try {
      renameWithRetry(sourcePath, targetPath);
    } catch (e) {
      if (process.platform === "win32" && (e.code === "EPERM" || e.code === "EACCES" || e.code === "ENOTEMPTY")) {
        if (!copyDirRecursively(sourcePath, targetPath)) {
          throw e;
        }
    ...
    if (movedExistingTarget && !fs.existsSync(targetPath) && fs.existsSync(backupPath)) {
      try {
        ...
        copyDirRecursively(backupPath, targetPath);
      }
    }
    throw error;
  }
