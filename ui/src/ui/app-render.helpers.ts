...
import { isAutoSpeakEnabled, setAutoSpeak, stopTts } from "./chat/speech.ts";
...
export function renderChatControls(state: AppViewState) {
  ...
  return html`
    <div class="chat-controls">
      ...
      <button
        class="btn btn--sm btn--icon ${isAutoSpeakEnabled() ? "active" : ""}"
        @click=${() => {
      ...
      >
        ${isAutoSpeakEnabled() ? icons.volume2 : icons.volumeOff}
      </button>
    </div>
  `;
}
