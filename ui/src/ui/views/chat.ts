...
import { isSttSupported, startStt, stopStt, isTtsSupported, isAutoSpeakEnabled, speakText, addToChunkBuffer, setAutoSpeak, initializeTts } from "../chat/speech.ts";
...
interface ChatEphemeralState {
  ...
  sttContinuousMode: boolean;
  sttSilenceTimer: NodeJS.Timeout | null;
  sttLastTranscriptTime: number;
  sttWaitingForSend: boolean;
...
function createChatEphemeralState(): ChatEphemeralState {
  return {
    sttRecording: false,
    sttInterimText: "",
    sttContinuousMode: false,
    sttSilenceTimer: null,
    sttLastTranscriptTime: 0,
    sttWaitingForSend: false,
...
export function resetChatViewState() {
  if (vs.sttRecording || vs.sttContinuousMode) {
    stopStt();
    cancelSilenceTimer();
  }
...
/**
 * Check if text is valid for auto-sending.
 * - Minimum 3 characters (simple threshold)
 */
function isValidSendText(text: string): boolean {
  const trimmed = text.trim();
  // Just check minimum length - don't reject based on word endings
  return trimmed.length >= 3;
}

/**
 * Cancel the existing silence timer if running.
 */
function cancelSilenceTimer(): void {
  if (vs.sttSilenceTimer) {
    clearTimeout(vs.sttSilenceTimer);
    vs.sttSilenceTimer = null;
  }
}
...
/**
 * Check if a new assistant message appeared and trigger auto-TTS if enabled.
 */
function processAutoSpeak(messages: unknown[], requestUpdate: () => void): void {
  if (!isTtsSupported() || !isAutoSpeakEnabled()) {
    return;
  }
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (msg && typeof msg === "object") {
      ...
      if (role === "assistant") {
        ...
        msgId = (msg as any).id || (msg as any).key || `msg-${i}`;
        break;
      }
    }
  }

  if (!lastAssistantMsg || !msgId) {
    return;
  }

  // Only speak if this is a new message we haven't spoken yet
  if (vs.lastSpokenMessageId === msgId) {
    return;
  }

  vs.lastSpokenMessageId = msgId;

  const content = (lastAssistantMsg as any).content;
  if (typeof content === "string" && content.trim()) {
    console.log("[AUTO-SPEAK] Speaking new assistant message:", content.slice(0, 100));
    addToChunkBuffer(content);
  }
}
...
export function renderChat(props: ChatProps) {
  // Initialize TTS system on first render (loads state and voices)
  if (isTtsSupported()) {
    initializeTts();
  }
  ...
  processAutoSpeak(props.messages, requestUpdate);
        ...
        <div class="agent-chat__toolbar">
          <div class="agent-chat__toolbar-left">
            ...
            ${isSttSupported()
              ? html`
                  <button
                    class="agent-chat__input-btn ${vs.sttContinuousMode
                      ? "agent-chat__input-btn--recording"
                      : ""}"
                    @click=${() => {
                      if (vs.sttContinuousMode) {
                        // Stop continuous listening mode
                        console.log("[MIC-BTN] STOPPING continuous mode");
                        stopStt();
                        cancelSilenceTimer();
                        vs.sttContinuousMode = false;
                        vs.sttWaitingForSend = false;
                        vs.sttRecording = false;
                        vs.sttInterimText = "";
                        requestUpdate();
                      } else {
                        // Start continuous listening mode
                        console.log("[MIC-BTN] STARTING continuous mode");
                        const startContinuousListening = () => {
                          const started = startStt({
                            onTranscript: (text, isFinal) => {
                              if (isFinal) {
                                // Got final transcript - append to draft
                                const current = getDraft();
                                const sep = current && !current.endsWith(" ") ? " " : "";
                                const newDraft = current + sep + text;
                                props.onDraftChange(newDraft);
                                vs.sttInterimText = "";
                                vs.sttLastTranscriptTime = Date.now();
                                vs.sttWaitingForSend = true;
                                
                                // Cancel any existing silence timer
                                cancelSilenceTimer();
                                
                                // Start silence timer: if no more speech for 1.5 seconds, auto-send
                                console.log("[STT-FINAL]", text, "→ draft:", newDraft);
                                vs.sttSilenceTimer = setTimeout(() => {
                                  const finalDraft = getDraft();
                                  console.log("[SILENCE-TIMER] Fired! Draft:", finalDraft, "Valid:", isValidSendText(finalDraft), "Continuous:", vs.sttContinuousMode);
                                  if (vs.sttContinuousMode && isValidSendText(finalDraft)) {
                                    // Valid message - stop STT, then send it
                                    console.log("[AUTO-SEND] Sending:", finalDraft);
                                    stopStt();
                                    vs.sttRecording = false;
                                    inputHistory.push(finalDraft);
                                    props.onSend();
                                    console.log("[AUTO-SEND] onSend() called ✓");
                                    // After send, restart listening after draft clears
                                    setTimeout(() => {
                                      if (vs.sttContinuousMode) {
                                        vs.sttWaitingForSend = false;
                                        console.log("[AUTO-SEND] Restart listening");
                                        startContinuousListening();
                                      }
                                    }, 500);
                                  } else if (vs.sttContinuousMode && !isValidSendText(finalDraft)) {
                                    // Invalid text - just restart listening without sending
                                    console.log("[SILENCE-TIMER] Text too short, restarting");
                                    vs.sttWaitingForSend = false;
                                    if (!vs.sttRecording) {
                                      startContinuousListening();
                                    }
                                  } else {
                                    // Not in continuous mode anymore - clean up
                                    console.log("[SILENCE-TIMER] Not in continuous mode");
                                    vs.sttWaitingForSend = false;
                                  }
                                  vs.sttSilenceTimer = null;
                                }, 1500);
                              } else {
                                // Interim transcript - just show it while user speaks
                            ...
                            onStart: () => {
                              console.log("[STT-START] Recording started");
                              vs.sttRecording = true;
                              cancelSilenceTimer();
                              requestUpdate();
                            },
                            onEnd: () => {
                              console.log("[STT-END] Recording ended");
                              vs.sttRecording = false;
                              // STT ended naturally - only restart if NOT waiting for silence timer to send
                              if (vs.sttContinuousMode && !vs.sttWaitingForSend && !vs.sttSilenceTimer) {
                                setTimeout(() => {
                                  if (vs.sttContinuousMode && !vs.sttWaitingForSend) {
                                    startContinuousListening();
                                  }
                                }, 100);
                              }
                            ...
                            onError: () => {
                              console.log("[STT-ERROR] Speech recognition error");
                              vs.sttRecording = false;
                              vs.sttInterimText = "";
                              // On error, retry in continuous mode (unless waiting for send)
                              if (vs.sttContinuousMode && !vs.sttWaitingForSend && !vs.sttSilenceTimer) {
                                setTimeout(() => {
                                  if (vs.sttContinuousMode && !vs.sttWaitingForSend) {
                                    startContinuousListening();
                        ...
                        vs.sttContinuousMode = true;
                        console.log("[MIC-BTN] Continuous mode ON, starting STT");
                        startContinuousListening();
                      }
                    }}
                    title=${vs.sttContinuousMode ? "Stop continuous listening (Mic ON)" : "Start continuous listening"}
                    ?disabled=${!props.connected}
                  >
                    ${vs.sttContinuousMode ? icons.micOff : icons.mic}
                  </button>
                `
              : nothing}
